...
const char * const BITCOIN_CONF_FILENAME = "suppo.conf";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "suppo" is a composite category enabling all Suppocoin-related debug output
            if(ptrCategory->count(string("suppo"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty suppo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override suppo.conf

...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "suppo.conf"), 'w') as f:
        f.write("regtest=1\n")
        f.write("rpcuser=rt\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for suppod to start. This means that RPC is accessible and fully initialized.
    Raise an exception if suppod exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('suppod exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir):
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        for i in range(4):
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                print "initialize_chain: suppod started, waiting for RPC to come up"
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in suppo.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """
    Start a suppod and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("SUPPOD", "suppod")
    ...
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print "start_node: suppod started, waiting for RPC to come up"
